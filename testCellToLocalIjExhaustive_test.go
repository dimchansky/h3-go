...
func localIjToH3_identity_assertions(t *testing.T, h3 H3Index) {
	...
	if h3 != retrieved {
		t.Errorf("round trip through local IJ space failed for %#016x: got %#016x", h3, retrieved)
	}
}
...
func h3ToLocalIj_neighbors_assertions(t *testing.T, h3 H3Index) {
	...
	for d := K_AXES_DIGIT; d < INVALID_DIGIT; d++ {
		if d == K_AXES_DIGIT && isPentagon(h3) {
			continue
		}
...
func h3ToLocalIj_invalid_assertions(t *testing.T, h3 H3Index) {
	...
	for i := int64(0); i < sz; i++ {
		if neighbors[i] == H3_NULL {
			continue
		}

		var ij CoordIJ
		// Don't consider indexes which we can't unfold in the first place
		if cellToLocalIj(h3, neighbors[i], 0, &ij) == E_SUCCESS {
			for j := 0; j < 2; j++ {
				var dir Direction
				if j == 0 {
					dir = INVALID_DIGIT
			...
			if reverseFailed == E_SUCCESS {
				if expectedIj.I != ij.I || expectedIj.J != ij.J {
					...
					if testH3 != testTestH3 {
						t.Errorf("index has normalizable coordinates in local IJ failed for %#016x: testH3=%#016x != testTestH3=%#016x", h3, testH3, testTestH3)
					}
				}
			}
		}
