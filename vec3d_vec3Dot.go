...
// does fuse on arm64 (even across statements). The parity harness
// compiles the reference C with contraction disabled
// (-ffp-contract=off; see the test-c2go comment in the Makefile), so
// bit-exact parity requires the same uncontracted evaluation on the Go
// side, which these conversions guarantee.
