Scope
- Track C→Go function conversions.
...
- Transpile: implement a faithful Go port in `<cfile>_<function>.go`, mirroring names/signatures (unexported) and behavior (NO build tag for pure Go).
...
- Parity test: add `<cfile>_<function>_parity_test.go` with `//go:build cgo` that compares Go vs C output; use tight, justified tolerances for floats and safe bool handling.
...
- Use descriptive prefixes like `c2go:`, `vertex:`, `faceijk:` etc.
...
```
c2go: port vertexRotations function with parity
