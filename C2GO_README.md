...
Conventions
- One function per Go file: `<cfile>_<function>.go` (unexported, keep original name when possible).
...
Build Tags
- **Pure Go implementation files** (`<cfile>_<function>.go`): NO build tags needed
- **CGO interop files** (`*_cgo.go`): Must have `//go:build cgo` as the first line
- **Parity test files** (`*_parity_test.go`): Must have `//go:build cgo` as the first line
- This ensures pure Go implementations are always available, while CGO-dependent code only builds when explicitly enabled

Testing
- Parity tests compare the Go port vs the C original and require CGO to be enabled.
- Parity test naming: `<cfile>_<function>_parity_test.go` (reserved `<cfile>_<function>_test.go` for plain-Go tests later).
- CGO is only used in `*_cgo.go` files and parity tests, never in pure Go implementations.
...
- Create `<cfile>_<function>.go` with a faithful Go translation; keep the same unexported name (NO build tag).
- Add/extend `<cfile>_cgo.go`:
  - `//go:build cgo` as the first line
...
- Add `<cfile>_<function>_parity_test.go` with `//go:build cgo` that compares Go vs C wrappers.
...
- Library package remains pure Go. cgo exists only in `*_cgo.go` and only builds with the `c2go` tag.
