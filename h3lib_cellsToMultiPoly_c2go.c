...
// Direct createSortablePoly on the sorted loop set: the polygon whose
// outer loop starts at loopStart with numHoles holes.
H3Error h3goTest_createSortablePoly(const H3Index *cells, int64_t numCells,
    ...
    SortablePoly spoly;
    err = createSortablePoly(&loopset.sloops[loopStart], numHoles, &spoly);
    if (err) {
        destroySortableLoopSet(&loopset);
        destroyArcSet(&arcset);
        return err;
    ...
    *outerNumVertsOut = spoly.poly.geoloop.numVerts;
    ...
    for (int64_t h = 0; h < numHoles; h++) {
        ...
        for (int64_t i = 0; i < spoly.poly.holes[h].numVerts; i++) {
            holeVerts[v++] = spoly.poly.holes[h].verts[i];
        }
    }
...
// Direct createMultiPolygon (isolated from validate/overflow): run the
// pipeline to the sorted loop set, call createMultiPolygon only, and
// serialize the result. Buffers as for h3goTest_loopSet plus per-poly
// hole counts.
H3Error h3goTest_createMultiPolygonOnly(const H3Index *cells,
    ...
    err = createMultiPolygon(loopset, &mpoly);
    if (err) {
        destroySortableLoopSet(&loopset);
        destroyArcSet(&arcset);
        return err;
    ...
    *numPolysOut = mpoly.numPolygons;
    ...
    for (int p = 0; p < mpoly.numPolygons; p++) {
        ...
        for (int i = 0; i < mpoly.polygons[p].geoloop.numVerts; i++) {
            verts[v++] = mpoly.polygons[p].geoloop.verts[i];
        }
        for (int k = 0; k < mpoly.polygons[p].numHoles; k++) {
            ...
            for (int i = 0; i < mpoly.polygons[p].holes[k].numVerts; i++) {
                verts[v++] = mpoly.polygons[p].holes[k].verts[i];
            }
        }
    }
