...
func Test_h3RotatePent60ccw_parity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			goResult := _h3RotatePent60ccw(tt.h)
			// Test the C implementation
			cResult := h3RotatePent60ccwC(tt.h)

			if goResult != cResult {
				t.Errorf("_h3RotatePent60ccw() parity mismatch: got %016x, want %016x", goResult, cResult)
			}
		})
	}
}

func Test_h3RotatePent60ccw_all_pentagon_base_cells(t *testing.T) {
	...
	for _, baseCell := range pentagonBaseCells {
		for res := int32(0); res <= 5; res++ { // Test lower resolutions to keep tests fast
			t.Run(fmt.Sprintf("baseCell_%d_res_%d", baseCell, res), func(t *testing.T) {
				...
				goResult := _h3RotatePent60ccw(h)
				cResult := h3RotatePent60ccwC(h)

				if goResult != cResult {
					t.Errorf("_h3RotatePent60ccw() parity mismatch for baseCell %d res %d: got %016x, want %016x", baseCell, res, goResult, cResult)
				}
			})
		}
	}
