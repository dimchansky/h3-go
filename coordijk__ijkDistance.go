package h3

// abs32 returns the absolute value of x for int32.
func abs32(x int32) int32 {
	if x < 0 {
		return -x
	}
	return x
}

// max32 returns the maximum of a and b for int32.
func max32(a, b int32) int32 {
	if a > b {
		return a
	}
	return b
}

// ijkDistance computes the distance between two IJK coordinates.
// Mirrors H3's coordijk.h::ijkDistance behavior.
// Ported from H3 C: coordijk.h::ijkDistance.
func ijkDistance(c1, c2 *coordIJK) int32 {
	var diff coordIJK
	_ijkSub(c1, c2, &diff)
	_ijkNormalize(&diff)
	absDiff := coordIJK{I: abs32(diff.I), J: abs32(diff.J), K: abs32(diff.K)}
	return max32(absDiff.I, max32(absDiff.J, absDiff.K))
}
