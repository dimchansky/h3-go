...
// Finding: Both C and Go implementations return eFailed for degenerate polygons
// at the origin, but return eSuccess for polygons with actual coordinates.
func Test_maxPolygonToCellsSize_degeneratePolygons_parity(t *testing.T) {
	t.Parallel()

	// Point polygon with single vertex at origin
	// Both C and Go return eFailed for this case
	...
	// Both C and Go return eSuccess for this case
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			} else if goErr == eSuccess && goNumHexagons != cNumHexagons {
				t.Errorf("Size mismatch: Go returned %d, C returned %d", goNumHexagons, cNumHexagons)
			} else {
				// Success - behaviors match
				if goErr == eFailed {
					t.Logf("Both correctly reject degenerate polygon with eFailed")
				} else {
