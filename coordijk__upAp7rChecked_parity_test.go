...
func Test_upAp7rChecked_parity(t *testing.T) {
	tests := []struct {
		name  string
		coord coordIJK
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			coordC := tt.coord
			coordCResult, errC := _upAp7rCheckedC(&coordC)

			// Call Go implementation
			coordGo := tt.coord
			errGo := _upAp7rChecked(&coordGo)

			// Compare errors first
			if errGo != errC {
				t.Errorf("_upAp7rChecked() error mismatch: Go=%v != C=%v for input{%d,%d,%d}",
			...
			if errGo == eSuccess {
				if coordGo.I != coordCResult.I || coordGo.J != coordCResult.J || coordGo.K != coordCResult.K {
					t.Errorf("_upAp7rChecked() mismatch: Go{%d,%d,%d} != C{%d,%d,%d} for input{%d,%d,%d}",
	...
	t.Run("deterministic", func(t *testing.T) {
		...
		result1 := coord
		err1 := _upAp7rChecked(&result1)

		result2 := coord
		err2 := _upAp7rChecked(&result2)

		if err1 != err2 {
			t.Errorf("_upAp7rChecked should return same error: first=%v != second=%v", err1, err2)
		}

		if err1 == eSuccess &&
			(result1.I != result2.I || result1.J != result2.J || result1.K != result2.K) {
			t.Errorf("_upAp7rChecked should be deterministic: first{%d,%d,%d} != second{%d,%d,%d}",
	...
	t.Run("overflow detection", func(t *testing.T) {
		overflowCases := []coordIJK{
			...
			{2147483647, 0, 0}, // int32Max
			{0, 2147483647, 0}, // int32Max
		}
		...
		for _, coord := range overflowCases {
			...
			coordC := coord
			_, errC := _upAp7rCheckedC(&coordC)

			if errGo != errC {
				t.Errorf("_upAp7rChecked() overflow detection mismatch: Go=%v != C=%v for {%d,%d,%d}",
