// Tests ported from H3 v4.4.0: src/apps/testapps/testVec3dInternal.c.
package h3
...
func Test_geoToVec3d(t *testing.T) {
	...
	var p1 vec3d
	_geoToVec3d(c1, &p1)
	if math.Abs(_pointSquareDist(origin, &p1)-1) >= epsilonRad {
	...
	var p2 vec3d
	_geoToVec3d(c2, &p2)
	if math.Abs(_pointSquareDist(&p1, &p2)-2) >= epsilonRad {
	...
	var p3 vec3d
	_geoToVec3d(c3, &p3)
	if math.Abs(_pointSquareDist(&p1, &p3)-4) >= epsilonRad {
