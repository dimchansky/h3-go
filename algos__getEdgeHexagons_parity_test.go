...
func Test_getEdgeHexagons_parity(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			geoloop:     []LatLng{{Rad(0.1), Rad(0.1)}, {Rad(0.1), Rad(0.2)}, {Rad(0.2), Rad(0.15)}},
			numHexagons: 100,
			res:         9,
		...
		{
			name:        "square loop",
			geoloop:     []LatLng{{Rad(0.0), Rad(0.0)}, {Rad(0.0), Rad(0.1)}, {Rad(0.1), Rad(0.1)}, {Rad(0.1), Rad(0.0)}},
			numHexagons: 200,
			res:         8,
		...
		},
		{
			...
			geoloop:     []LatLng{{Rad(0.5), Rad(-0.5)}},
			numHexagons: 50,
			res:         7,
...
func createPentagonLoop(centerLat, centerLng, radius float64) []LatLng {
	...
	for i := 0; i < 5; i++ {
		...
		loop[i] = LatLng{
			Lat: Rad(centerLat + radius*math.Cos(theta)),
			Lng: Rad(centerLng + radius*math.Sin(theta)),
		}
	}
	return loop
}
