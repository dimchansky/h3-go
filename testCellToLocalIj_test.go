...
func Test_cellToLocalIjFailed(t *testing.T) {
	...
	if err != eSuccess {
		t.Errorf("found quadIJ (1) failed: %v", err)
	}
	if ij.I != 0 || ij.J != 0 {
	...
	if err != eSuccess {
		t.Errorf("found quadIJ (2) failed: %v", err)
	}
	if ij.I != 1 || ij.J != 0 {
	...
	if err != eSuccess {
		t.Errorf("found quadIJ (3) failed: %v", err)
	}
	if ij.I != 0 || ij.J != -1 {
	...
	if err != eSuccess {
		t.Errorf("found quadIJ (4) failed: %v", err)
	}
	if ij.I != -1 || ij.J != 0 {
...
func Test_cellToLocalIjInvalid(t *testing.T) {
	...
	if err != eCellInvalid {
		t.Errorf("invalid index: expected eCellInvalid, got %v", err)
	}

	// Test invalid origin
	err = cellToLocalIj(0x7fffffffffffffff, bc1, 0, &ij)
	if err != eResMismatch {
		t.Errorf("invalid origin: expected eResMismatch, got %v", err)
	}

	// Test invalid origin and index
	err = cellToLocalIj(0x7fffffffffffffff, 0x7fffffffffffffff, 0, &ij)
	if err != eCellInvalid {
		t.Errorf("invalid origin and index: expected eCellInvalid, got %v", err)
	}
}
...
func Test_localIjToCellInvalid(t *testing.T) {
	...
	if err != eCellInvalid {
		t.Errorf("invalid origin for ijToH3: expected eCellInvalid, got %v", err)
	}
}
...
func Test_indexOnPentInvalid(t *testing.T) {
	...
	// Tests for invalidDigit being detected and failed on in various cases.
	var onPentInvalid h3Index
	setH3Index(&onPentInvalid, 1, 4, int32(invalidDigit))

	var offPent h3Index
	setH3Index(&offPent, 1, 3, int32(centerDigit))

	var ij CoordIJ
	err := cellToLocalIj(offPent, onPentInvalid, 0, &ij)
	if err != eCellInvalid {
		t.Errorf("invalid index on pentagon: expected eCellInvalid, got %v", err)
	}

	var onPentValid h3Index
	setH3Index(&onPentValid, 1, 4, int32(centerDigit))

	err = cellToLocalIj(onPentInvalid, onPentValid, 0, &ij)
	if err != eCellInvalid {
		t.Errorf("invalid both on pentagon (1): expected eCellInvalid, got %v", err)
	}

	err = cellToLocalIj(onPentValid, onPentInvalid, 0, &ij)
	if err != eCellInvalid {
		t.Errorf("invalid both on pentagon (2): expected eCellInvalid, got %v", err)
	}
	...
	if err != eCellInvalid {
		t.Errorf("invalid both on pentagon (3): expected eCellInvalid, got %v", err)
	}
	...
	err = localIjToCell(onPentInvalid, &ij, 0, &out)
	if err != eCellInvalid {
		t.Errorf("invalid origin on pentagon: expected eCellInvalid, got %v", err)
	}
}
...
func Test_invalidMode(t *testing.T) {
	...
	for i := uint32(1); i <= 32; i++ {
		...
		if err != eOptionInvalid {
			t.Errorf("Invalid mode fail for cellToLocalIj at mode %d: expected eOptionInvalid, got %v", i, err)
		}

		var cell2 h3Index
		err = localIjToCell(cell, &ij2, i, &cell2)
		if err != eOptionInvalid {
			t.Errorf("Invalid mode fail for localIjToCell at mode %d: expected eOptionInvalid, got %v", i, err)
		}
	}
}
