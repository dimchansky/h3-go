...
func Test_cellAreaRads2_parity(t *testing.T) {
	tests := []struct {
		name string
		cell h3Index
	}{
	...
	const tolerance = 1e-15 // Very tight tolerance for area calculations

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Get C implementation result
			cArea, cErr := cellAreaRads2C(tt.cell)

			// Get Go implementation result
			goArea, goErr := cellAreaRads2(tt.cell)
			...
			// Compare areas with tight tolerance
			if !floatAlmostEqual(cArea, goArea, tolerance) {
				t.Errorf("Area mismatch for cell %016x: C=%.17e, Go=%.17e, diff=%.2e",
