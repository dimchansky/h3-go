...
func Test_h3Rotate60cw_parity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			goResult := _h3Rotate60cw(tt.h)
			// Test the C implementation
			cResult := h3Rotate60cwC(tt.h)

			if goResult != cResult {
				t.Errorf("_h3Rotate60cw() parity mismatch: got %016x, want %016x", goResult, cResult)
			}
		})
	}
}
...
func Test_h3Rotate60cw_all_resolutions(t *testing.T) {
	...
	for i, h := range testIndices {
		t.Run(fmt.Sprintf("resolution_%d", i), func(t *testing.T) {
			goResult := _h3Rotate60cw(h)
			cResult := h3Rotate60cwC(h)

			if goResult != cResult {
				t.Errorf("_h3Rotate60cw() parity mismatch at resolution %d: got %016x, want %016x", i, goResult, cResult)
			}
		})
	}
}
