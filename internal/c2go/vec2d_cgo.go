...
func v2dIntersectC(p0, p1, p2, p3 Vec2d) Vec2d {
    ...
    c0.x = C.double(p0.X); c0.y = C.double(p0.Y)
    c1.x = C.double(p1.X); c1.y = C.double(p1.Y)
    c2.x = C.double(p2.X); c2.y = C.double(p2.Y)
    c3.x = C.double(p3.X); c3.y = C.double(p3.Y)
...
func v2dAlmostEqualsC(v1, v2 Vec2d) bool {
    var c1, c2 C.Vec2d
    c1.x = C.double(v1.X); c1.y = C.double(v1.Y)
    c2.x = C.double(v2.X); c2.y = C.double(v2.Y)
