...
Build Tags
- **Pure Go implementation files** (`<cfile>__<function>.go`): NO build tags needed
...
- Create `internal/c2go/<cfile>__<function>.go` with a faithful Go translation; keep the same unexported name (NO build tag).
...
- Add `internal/c2go/<cfile>__<function>_parity_test.go` with `//go:build cgo` that compares Go vs C wrappers.
