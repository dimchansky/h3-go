...
func Test_downAp7r_parity(t *testing.T) {
	tests := []struct {
		name  string
		coord CoordIJK
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if gotGo.I != gotC.I || gotGo.J != gotC.J || gotGo.K != gotC.K {
				t.Errorf("_downAp7r() mismatch: Go{%d,%d,%d} != C{%d,%d,%d} for input{%d,%d,%d}",
	...
	t.Run("deterministic", func(t *testing.T) {
		...
		if result1.I != result2.I || result1.J != result2.J || result1.K != result2.K {
			t.Errorf("_downAp7r should be deterministic: first{%d,%d,%d} != second{%d,%d,%d}",
	...
	t.Run("up_down_relationship", func(t *testing.T) {
		...
		for _, coord := range testCoords {
			// Apply down then up (clockwise versions)
			down := coord
			_downAp7r(&down)
			upDown := down
			_upAp7r(&upDown)
