...
func TestOracle_CoordIJKRotate(t *testing.T) {
    ...
    for _, v := range cases {
        got := v; got.Rotate60CCW(); arr := o.IJKRotate60ccw([3]int{v.I,v.J,v.K}); want := CoordIJK{arr[0],arr[1],arr[2]}
        if got != want { t.Fatalf("Rotate60CCW(%v) = %v, want %v", v, got, want) }
        got = v; got.Rotate60CW(); arr = o.IJKRotate60cw([3]int{v.I,v.J,v.K}); want = CoordIJK{arr[0],arr[1],arr[2]}
...
func TestOracle_CoordIJKNeighbor(t *testing.T) {
    ...
    for d := Direction(0); d < NumDigits; d++ {
        got := v; got.Neighbor(d); arr := o.Neighbor([3]int{v.I,v.J,v.K}, int(d)); want := CoordIJK{arr[0],arr[1],arr[2]}
    ...
    for i := 0; i < n; i++ {
        ...
        for d := Direction(0); d < NumDigits; d++ {
            got := base; got.Neighbor(d); arr := o.Neighbor([3]int{base.I, base.J, base.K}, int(d)); want := CoordIJK{arr[0], arr[1], arr[2]}
