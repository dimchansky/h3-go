...
func TestOracle_CoordIJKDistance(t *testing.T) {
    ...
    for _, tc := range cases {
        if Distance(tc.a, tc.b) != o.DistanceIJK([3]int{tc.a.I,tc.a.J,tc.a.K}, [3]int{tc.b.I,tc.b.J,tc.b.K}) {
...
func TestOracle_CoordIJKRotate(t *testing.T) {
    ...
    for _, v := range cases {
        got := v; got.Rotate60CCW(); arr := o.RotateIJKCCW([3]int{v.I,v.J,v.K}); want := CoordIJK{arr[0],arr[1],arr[2]}
        if got != want { t.Fatalf("Rotate60CCW(%v) = %v, want %v", v, got, want) }
        got = v; got.Rotate60CW(); arr = o.RotateIJKCW([3]int{v.I,v.J,v.K}); want = CoordIJK{arr[0],arr[1],arr[2]}
...
func TestOracle_CoordIJKNeighbor(t *testing.T) {
    ...
    for d := Direction(0); d < NumDigits; d++ {
        got := v; got.Neighbor(d); arr := o.NeighborIJK([3]int{v.I,v.J,v.K}, int(d)); want := CoordIJK{arr[0],arr[1],arr[2]}
    ...
    for i := 0; i < n; i++ {
        ...
        for d := Direction(0); d < NumDigits; d++ {
            got := base; got.Neighbor(d); arr := o.NeighborIJK([3]int{base.I, base.J, base.K}, int(d)); want := CoordIJK{arr[0], arr[1], arr[2]}
                ...
                for ib := -3; ib <= 3 && count < maxPairs; ib++ {
                    for jb := -3; jb <= 3 && count < maxPairs; jb++ {
                        for kb := -3; kb <= 3 && count < maxPairs; kb++ {
                            b := CoordIJK{ib, jb, kb}
                            if Distance(a, b) != o.DistanceIJK([3]int{a.I,a.J,a.K}, [3]int{b.I,b.J,b.K}) {
