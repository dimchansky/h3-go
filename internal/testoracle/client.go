...
func (c *Client) IJKDistance(a, b [3]int) int {
	...
	if err != nil {
		c.t.Fatalf("parse distance: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) IJKRotate60ccw(v [3]int) [3]int {
	...
	if len(f) != 3 {
		c.t.Fatalf("unexpected rotate ccw output: %q", out)
	}
...
func (c *Client) IJKRotate60cw(v [3]int) [3]int {
	...
	f := strings.Fields(out)
	if len(f) != 3 {
		c.t.Fatalf("unexpected rotate cw output: %q", out)
	}
	i, _ := strconv.Atoi(f[0])
	j, _ := strconv.Atoi(f[1])
	k, _ := strconv.Atoi(f[2])
...
func (c *Client) Neighbor(v [3]int, d int) [3]int {
	...
	f := strings.Fields(out)
	if len(f) != 3 {
		c.t.Fatalf("unexpected neighbor output: %q", out)
	}
	i, _ := strconv.Atoi(f[0])
	j, _ := strconv.Atoi(f[1])
	k, _ := strconv.Atoi(f[2])
...
func (c *Client) ijkCmd(cmd string, v [3]int) [3]int {
	...
	f := strings.Fields(out)
	if len(f) != 3 {
		c.t.Fatalf("unexpected %s output: %q", cmd, out)
	}
	i, _ := strconv.Atoi(f[0])
	j, _ := strconv.Atoi(f[1])
	k, _ := strconv.Atoi(f[2])
...
func (c *Client) IJKToHex2d(v [3]int) (float64, float64) {
	...
	f := strings.Fields(out)
	if len(f) != 2 {
		c.t.Fatalf("unexpected ijkToHex2d output: %q", out)
	}
	x, err1 := strconv.ParseFloat(f[0], 64)
	y, err2 := strconv.ParseFloat(f[1], 64)
	if err1 != nil || err2 != nil {
		c.t.Fatalf("parse hex2d: %q", out)
	}
	return x, y
}
...
func (c *Client) Hex2dToCoordIJK(x, y float64) [3]int {
	...
	f := strings.Fields(out)
	if len(f) != 3 {
		c.t.Fatalf("unexpected hex2dToCoordIJK output: %q", out)
	}
	i, _ := strconv.Atoi(f[0])
	j, _ := strconv.Atoi(f[1])
	k, _ := strconv.Atoi(f[2])
...
func (c *Client) H3Rotate60ccw(h uint64) uint64 {
	...
	if err != nil {
		c.t.Fatalf("parse rotate60ccw: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) H3Rotate60cw(h uint64) uint64 {
	...
	v, err := strconv.ParseUint(strings.TrimPrefix(out, "0x"), 16, 64)
	if err != nil {
		c.t.Fatalf("parse rotate60cw: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) FaceIjkToH3(face, i, j, k, res int) uint64 {
	...
	v, err := strconv.ParseUint(strings.TrimPrefix(out, "0x"), 16, 64)
	if err != nil {
		c.t.Fatalf("parse faceijk h3: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) LatLngToCell(lat, lng float64, res int) (uint64, int) {
	...
	f := strings.Fields(out)
	if len(f) != 2 {
		c.t.Fatalf("unexpected latlng output: %q", out)
	}
	h3, err := strconv.ParseUint(strings.TrimPrefix(f[0], "0x"), 16, 64)
	if err != nil {
		c.t.Fatalf("parse latlng h3: %q: %v", out, err)
	}
	code, err2 := strconv.Atoi(f[1])
	if err2 != nil {
		c.t.Fatalf("parse latlng errcode: %q: %v", out, err2)
	}
	return h3, code
}
...
func (c *Client) GetResolution(h uint64) int {
	...
	v, err := strconv.Atoi(out)
	if err != nil {
		c.t.Fatalf("parse getResolution: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) GetBaseCellNumber(h uint64) int {
	...
	v, err := strconv.Atoi(out)
	if err != nil {
		c.t.Fatalf("parse getBaseCellNumber: %q: %v", out, err)
	}
	return v
}
...
func (c *Client) IsBaseCellPentagon(base int) bool {
	...
	v, err := strconv.Atoi(out)
	if err != nil {
		c.t.Fatalf("parse isBaseCellPentagon: %q: %v", out, err)
	}
	return v != 0
}
...
func (c *Client) GeoToHex2d(lat, lng float64, res int) (int, float64, float64) {
	...
	f := strings.Fields(out)
	if len(f) != 3 {
		c.t.Fatalf("unexpected geoToHex2d output: %q", out)
	}
