// Package faceijk implements face-centered IJK coordinate transformations and
// related geometry for mapping lat/lng to H3 indices using the icosahedron's
// local Face+IJK coordinate system.
...
// FaceCenterPoint are the precomputed 3D unit vectors for each face center
// (parity with H3 C faceCenterPoint table). Using these reduces numerical
// differences in face selection near borders.
var FaceCenterPoint = [NumIcosaFaces][3]float64{
...
func GeoToClosestFace(lat, lng float64) (face int, sqDist float64) {
	...
	for f := 0; f < NumIcosaFaces; f++ {
		fx, fy, fz := FaceCenterPoint[f][0], FaceCenterPoint[f][1], FaceCenterPoint[f][2]
		dx := x - fx
		dy := y - fy
		dz := z - fz
		d := dx*dx + dy*dy + dz*dz
...
// ToH3 converts FaceIJK coordinates to an H3 index.
// This implements the H3 C _faceIjkToH3 function logic.
func ToH3(fijk FaceIJK, res int) uint64 {
	// Initialize H3 index
	h := indexbits.H3_INIT
