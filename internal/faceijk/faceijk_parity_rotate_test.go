...
func TestOracle_H3Rotate_PentagonCases(t *testing.T) {
	...
	for _, p := range pentFaces {
		h := o.FaceIjkToH3(p.face, p.i, p.j, p.k, 0)
		if h == 0 {
			t.Fatalf("oracle pentagon faceijk produced 0")
		}
		if got := h3Rotate60ccw(h); got != o.H3Rotate60ccw(h) {
			t.Fatalf("pent ccw mismatch for 0x%x", h)
		}
		if got := h3Rotate60cw(h); got != o.H3Rotate60cw(h) {
