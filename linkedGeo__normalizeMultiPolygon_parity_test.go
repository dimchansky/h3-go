...
func Test_normalizeMultiPolygon_parity(t *testing.T) {
	...
	createSquareLoopCCW := func(lat, lng, size float64) *linkedGeoLoop {
		...
		coords := []LatLng{
			...
			{Lat: Rad(lat + size), Lng: Rad(lng + size)}, // Move north to NE corner
			{Lat: Rad(lat + size), Lng: Rad(lng)},        // Move west to NW corner
	...
	createSquareLoopCW := func(lat, lng, size float64) *linkedGeoLoop {
		...
		coords := []LatLng{
			...
			{Lat: Rad(lat + size), Lng: Rad(lng + size)}, // Move east to NE corner
			{Lat: Rad(lat), Lng: Rad(lng + size)},        // Move south to SE corner
