...
operations over identical deterministic inputs. The generated
[complete results](results.md) show every measured scenario; broader API and
behavioral interpretation lives in
[comparison-uber-h3-go.md](../comparison-uber-h3-go.md). Every published
number must remain traceable to an artifact here.
...
- `benchstat.txt` — [benchstat](https://pkg.go.dev/golang.org/x/perf/cmd/benchstat)
  ...
  A/B ratios and p-values). **This is the source for the generated complete
  comparison and README scorecard.**
...
The generated [complete comparison](results.md) presents all 33 scenarios for
both environments at once. It includes execution time, Go-heap bytes and
allocations, all seven isolated process-memory workloads, reusable-buffer
modes, percentage advantages, confidence intervals, and links back to each
run's raw artifacts. The main README keeps only a compact all-scenario
scorecard.
