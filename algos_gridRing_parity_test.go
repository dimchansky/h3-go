...
func Test_gridRing_parity(t *testing.T) {
	...
	t.Run("pentagon_origin", func(t *testing.T) {
		...
		// gridRing should handle pentagons gracefully using the fallback
		// It may succeed where gridRingUnsafe would fail
	})

	// Test cells near pentagons - should handle gracefully
	t.Run("near_pentagon", func(t *testing.T) {
		...
		for _, tc := range testCases {
			...
			// Just compare errors - both should handle gracefully
			if goErr != cErr {
			...
			if goErr == E_SUCCESS {
				...
				// Compare maps
				if len(goMap) != len(cMap) {
					t.Errorf("Different number of non-zero cells near pentagon: Go=%d, C=%d", len(goMap), len(cMap))
				}
			}
		}
	})
