// Tests ported from H3 v4.4.0: src/apps/testapps/testCompactCells.c.
package h3
...
func TestCompactCells_duplicateMinimum(t *testing.T) {
	...
	if compactCellsResult != eDuplicateInput {
...
func TestCompactCells_disparate(t *testing.T) {
	...
	for i := 0; i < numHex; i++ {
		setH3Index(&disparate[i], 1, int32(i), int32(centerDigit))
	}
...
func TestCompactCells_reservedBitsSet(t *testing.T) {
	...
	if err != eCellInvalid {
		t.Errorf("compactCells should return eCellInvalid on bad input, got %v", err)
	}
}
...
func TestCompactCells_parentError(t *testing.T) {
	...
	err := compactCells(bad, output, numHex)
	if err != eResMismatch {
		t.Errorf("compactCells should return eResMismatch on bad input (parent error), got %v", err)
	}
}
...
func TestUncompactCells_wrongRes(t *testing.T) {
	...
	_, err = uncompactCellsSize(someHexagons, numHex, maxH3Res+1)
	if err != eResMismatch {
	...
	if uncompactCellsResult != eResMismatch {
	...
	if uncompactCellsResult != eMemoryBounds {
	...
	for i := int64(0); i < numHex; i++ {
		setH3Index(&someHexagons[i], maxH3Res, int32(i), 0)
	}
	uncompactCellsResult = uncompactCells(someHexagons, numHex, uncompressed, numHex*7, maxH3Res+1)
	if uncompactCellsResult != eResMismatch {
